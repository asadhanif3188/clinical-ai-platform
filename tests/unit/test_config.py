...
def base_env(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_URL", "postgresql+asyncpg://postgres:postgres@localhost:5432/clinical_ai")
    monkeypatch.setenv("PGVECTOR_DATABASE_URL", "postgresql+asyncpg://postgres:postgres@localhost:5432/clinical_ai")
...
def test_database_url_validator(base_env):
    ...
    with pytest.raises(ValidationError, match=r"DATABASE_URL must start with 'postgresql\+asyncpg://'"):
        Settings()
