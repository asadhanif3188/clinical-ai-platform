...
class AgentIdentity(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"name": "intake_agent", "version": "1.0.0"}}
    )


class ProvenanceRecord(BaseModel):
    document_id: UUID = Field(..., description="ID of the source document")
    page_number: int | None = Field(None, description="Page number where data was found")
    extraction_agent: str = Field(
        ..., description="Name of the agent that performed the extraction"
    )
    source_text: str | None = Field(None, description="Original text snippet from the document")
...
class ConfidenceScore(BaseModel):
    ...
    reasoning: str | None = Field(None, description="Explanation for the confidence score")
